...
function QuizContent({ data }: { data: TopicData }) {
  ...
  function handleNext() {
    if (currentQ < data.quiz.length - 1) {
      ...
      recordQuiz({ topic: data.id, score, date: new Date().toISOString() });
      showToast("xp-gain", "You just leveled up your brain.", `${score}/${data.quiz.length} correct`);
      setTimeout(() => router.push(`/boss/${data.id}`), 1200);
    }
  }
    ...
    <div className="min-h-screen flex flex-col safe-pb" style={{ background: "var(--bg-base)" }}>
      ...
      >
        ...
        <span className="font-heading text-sm tabular-nums" style={{ color: "var(--text-muted)" }}>
          {currentQ + 1} <span style={{ color: "var(--text-muted)", opacity: 0.4 }}>/ {data.quiz.length}</span>
        </span>
      </motion.header>
      ...
      <div className="h-0.5" style={{ background: "var(--bg-elevated)" }}>
        <motion.div
          className="h-full"
          style={{ background: "linear-gradient(90deg, var(--accent-indigo), #9C8FFF)", boxShadow: "0 0 6px var(--accent-indigo-glow)" }}
      ...
      <div className="flex-1 flex flex-col justify-center max-w-[640px] mx-auto w-full px-4 sm:px-6 py-8">
        <AnimatePresence mode="wait">
          ...
          >
            {/* Question */}
            <div className="mb-7">
              <p className="font-heading mb-3" style={{ fontSize: "11px", letterSpacing: "0.2em", textTransform: "uppercase", color: "var(--accent-indigo)" }}>
                Question {currentQ + 1}
              </p>
              <h2
                className="font-sans font-semibold leading-snug"
                style={{ fontSize: "clamp(17px, 4vw, 22px)", color: "var(--text-primary)", lineHeight: 1.5 }}
              >
                {question.question}
              </h2>
            </div>
              ...
              {answered && (
                ...
                >
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.97 }}
                    className="w-full mt-4 py-3.5 rounded-xl font-sans font-semibold text-sm text-white transition-all"
                    style={{
                      background: isLast
                  ...
                  >
                    {isLast ? "⚔ Enter the Boss Fight" : "Next Question →"}
                  </motion.button>
                </motion.div>
              )}
            </AnimatePresence>
